...
# jzx 改res2net改这里即可！kernel_size默认为3
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    # 1.取出上层输出，即x的四分之一的维度x1,x2,x3,x4

    # 2.输出即为：（加号代表通道上的拼接）y1+y2+y3+y4
    # y1 = x1
    # y2 = x2 * 3*3conv
    # y3 = (y2+x3) * 3*3conv
    # y4 = (y3+x4) * 3*3conv
    # x = concat(y1, y2, y3, y4)
    # 获取输入特征的通道
    ...
    for i in range(4):
        ...
        slice_x = KL.Lambda(lambda x: x[..., i * w:(i + 1) * w])(x)
        # 当第0组是时，x1不计算，直接保存特征到subset_x，
        # 当第1组时，x2经过3X3卷积后产生K2,保存到subset_x
        # 当第2组时，x3和从subset_x取出的K2,进行融合，再经过3X3卷积后产生K3,保存到subset_x
        # 当第3组时，同上步
        if i > 1:
            slice_x = KL.Add()([slice_x, subset_x[-1]])
        if i > 0:
            slice_x = KL.BatchNormalization()(slice_x)
            slice_x = KL.Activation('relu')(slice_x)
            slice_x = KL.Conv2D(w, 3, kernel_initializer='he_normal', padding='same', use_bias=False)(slice_x)
        subset_x.append(slice_x)
    # 将subset_x中保存的 y1，y2, y3, y4 进行 concat
    x = KL.Concatenate()(subset_x)
